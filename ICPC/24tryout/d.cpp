...
void solve() {
	...
	LazySegmentTree<T, F> seg2(n, 0, 0,
	    [](T a, T b) { return a + b; },
	    [](T a, F b, int len) { return a + b * len; },
	    [](F a, F b) { return a + b;}
	);
    for (int i = 0; i < m; i++) {
    	...
    	if (s == "+") {
    		...
    		cin >> b >> x;
    		x--;
    		seg.update(0, x, b * powMod(P, x));
    		seg2.update(x + 1, n - 1, b * powMod(P, n - x - 1));
    	} else if (s == "-") {
    		int b, x;
    		cin >> b >> x;
    		x--;
    		seg.update(0, x, 0 - b * powMod(P, x));
    		seg2.update(x + 1, n - 1, 0 - b * powMod(P, n - x - 1));
    	} else {
    		int x;
    		cin >> x;
    		x--;
    		mi ans = 0;
    		ans += seg.query(x, x) * powMod(iP, x);
    		ans += seg2.query(x, x) * powMod(iP, n - x - 1);
    		cout << (int) ans << endl;
    	}
    }
