...
struct LazySegmentTree {
    ...
    T find(int lo, int hi, int node, int node_lo, int node_hi, T x) {
        if (hi < node_lo || node_hi < lo) return -1;
        if (node_lo == node_hi) return node_lo;
        int mid = (node_lo + node_hi) / 2;
        T L = query(lo, hi, 2 * node, node_lo, mid);
        if (L < x) {
            T R = query(lo, hi, 2 * node + 1, mid + 1, node_hi);
            if (R < x) {
                return -1;
            }
            return find(lo, hi, 2 * node + 1, mid + 1, node_hi, x);
        } else {
            return find(lo, hi, 2 * node, node_lo, mid, x);
        }
    }

    T find(int lo, T x) {
        return find(lo, n - 1, 1, 0, n - 1, x);
    }
