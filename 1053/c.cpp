...
const ll MAXX = 1e16;
const int INF = 1e9 + 7;
// From https://usaco.guide/adv/segtree-beats?lang=cpp
...
void push_add(int t, int tl, int tr, ll v) {
	...
	T[t].sum += (tr - tl + 1) * v;
	T[t].max1 += v;
	if (T[t].max2 != -MAXX) { T[t].max2 += v; }
	T[t].min1 += v;
	if (T[t].min2 != MAXX) { T[t].min2 += v; }
	T[t].lazy += v;
}
...
void solve() {
    ...
    build();
	for (int i = n - 1; i >= 0; i--) {
		ll x = query_sum(c[i] + 1, c[i] + 1);
		update_add(1, c[i] + 1, w[i]);
		update_chmax(1, c[i] + 1, x);
	}
    cout << query_sum(1, 1) << endl;
    for (int i = 0; i <= n; i++) {
    	A[i] = 0;
    }
}
