...
struct SegmentTree {
    ...
    T query(int l, int r, int node = 1, int start = 0, int end = -1) {
        if (end == -1) end = n - 1;
        if (l > end || r < start) return -1;//NEED TO INITIALIZE THIS!!!!!  
...
void solve() {
    ...
    ll ans = 0;
    stack<pii> st;
    SegmentTree<int, function<int(int, int)>> seg(2 * n + 1, [](int a, int b) { return  max(a, b); }, -1);
	vi arr(2 * n + 1, -1);
	seg.build(arr);
    for (int i = 0; i < n; i++) {
    	if (a[i] == b[i]) {
    		...
    		int maxx = seg.query(max(a[i], b[i]), 2 * n);
    		ans += 1ll * (maxx + 1) * (n - i);
    	}
    	seg.update(a[i], i); 
    	//cout << i << " " << ans << endl;
